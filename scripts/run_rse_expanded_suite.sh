...
FGVC_SUBSET_ROOT="${FGVC_SUBSET_ROOT:-$PROJECT_ROOT/swap/subsets/20260324_125034_fgvc_method_suite}"

SWAP_ROOT="$PROJECT_ROOT/swap"
SUBSET_ROOT="$SWAP_ROOT/subsets/${TIMESTAMP}_rse_expanded_suite"
LOG_ROOT="$SWAP_ROOT/logs/${TIMESTAMP}_rse_expanded_suite"
OUTPUT_ROOT="$SWAP_ROOT/outputs/${TIMESTAMP}_rse_expanded_suite"
RECORD_ROOT="$SWAP_ROOT/records"
MANIFEST_PATH="$OUTPUT_ROOT/manifest.tsv"
SUMMARY_PATH="$RECORD_ROOT/rse_expanded_suite_${TIMESTAMP}.md"
...
METHOD_SPECS=(
  ...
  "sav|SAV|sav|"
  "rse|RSE|rse|"
  "rse_zscore|RSE-ZScore|rse|method.params.selection_metric=loo_accuracy;method.params.score_normalization=zscore"
  "rse_cv|RSE-CV|rse|method.params.selection_metric=cv_accuracy;method.params.cv_folds=5"
  "rse_shrink|RSE-Shrink|rse|method.params.selection_metric=loo_accuracy;method.params.centroid_shrinkage=auto"
  "rse_adaptive|RSE-Adaptive|rse|method.params.selection_metric=loo_accuracy;method.params.routing_mode=adaptive;method.params.adaptive_margin_quantile=0.6;method.params.score_normalization=zscore"
  "rse_pca|RSE-PCA|rse|method.params.selection_metric=loo_accuracy;method.params.feature_reduction=pca;method.params.pca_dim=64"
)

PREBUILT_EXPERIMENTS=(
  "cub|cub|raw|$FGVC_SUBSET_ROOT/cub/train_subset.json|$FGVC_SUBSET_ROOT/cub/val_subset.json"
  "eurosat|eurosat|raw|$FGVC_SUBSET_ROOT/eurosat/train_subset.json|$FGVC_SUBSET_ROOT/eurosat/val_subset.json"
)
...
for entry in "${PREBUILT_EXPERIMENTS[@]}"; do
  IFS='|' read -r experiment_id dataset_name evaluator_name train_subset val_subset <<< "$entry"
  if ! matches_dataset_filter "$experiment_id" "$dataset_name"; then
    continue
  fi
  if [[ ! -f "$train_subset" || ! -f "$val_subset" ]]; then
    echo "Missing prebuilt subset for ${experiment_id}: $train_subset or $val_subset" >&2
    exit 1
  fi
...
for entry in "${BUILT_EXPERIMENTS[@]}"; do
