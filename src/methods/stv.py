...
class STVMethod(MethodBase):
    ...
    def _generate_without_intervention(self, sample: Dict) -> str:
        conversation, flat_images = self._build_conversation([], sample, include_answer=False)
        inputs = self._prepare_inputs(conversation, flat_images, add_generation_prompt=True)
        prompt_len = int(inputs["input_ids"].shape[-1])
    ...
    def _run_write_failure_forward(self, full_inputs, *, prompt_len: int, steered: bool, sample: Dict):
        if steered:
            avg_activations = self._resolve_avg_activations(sample)
            self.hook_controller.enable_intervention(
                self.intervention_locations,
        ...
        if (
            ...
            raise RuntimeError(
                "STV write-failure analysis requires real attention tensors. "
                "Run with model.model_args.attn_implementation=eager."
            )
        return outputs

    def _get_write_failure_task_vector(self, sample: Dict) -> Dict[str, torch.Tensor] | None:
        avg_activations = self._resolve_avg_activations(sample)
        if avg_activations is None:
            return None
        return {
            "avg_activations": avg_activations.detach().to(device="cpu", dtype=torch.float32),
        }
    ...
    def _maybe_record_write_failure(
        ...
        *,
        ...
        normal_raw_output = self._generate_without_intervention(sample)
        ...
        prompt_len = int(prompt_inputs["input_ids"].shape[-1])
        normal_outputs = self._run_write_failure_forward(full_inputs, prompt_len=prompt_len, steered=False, sample=sample)
        steered_outputs = self._run_write_failure_forward(full_inputs, prompt_len=prompt_len, steered=True, sample=sample)
        ...
        self.write_failure_recorder.record_pair(
            ...
            task_vector_obj=self._get_write_failure_task_vector(sample),
